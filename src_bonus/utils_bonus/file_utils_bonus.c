...
/*   Updated: 2024/11/15 11:19:05 by gude-jes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_duplicate_text(t_game *game)
{
	if (game->textr.north == game->textr.south || game->textr.north
		...
		== game->textr.enemy1 || game->textr.north == game->textr.enemy2
		|| game->textr.north == game->textr.enemyd1 || game->textr.north
		== game->textr.enemyd2)
		sepuku(game, ERROR_TEXTURE);
	if (game->textr.south == game->textr.east
		|| game->textr.south == game->textr.west || game->textr.south
		== game->textr.door || game->textr.south == game->textr.enemy1
		|| game->textr.south == game->textr.enemy2 || game->textr.south
		== game->textr.enemyd1 || game->textr.south == game->textr.enemyd2)
		sepuku(game, ERROR_TEXTURE);
	if (game->textr.east == game->textr.west || game->textr.east
		== game->textr.door || game->textr.east == game->textr.enemy1
		|| game->textr.east == game->textr.enemy2 || game->textr.east
		== game->textr.enemyd1 || game->textr.east == game->textr.enemyd2)
		sepuku(game, ERROR_TEXTURE);
	check_duplicate_text2(game);
}
