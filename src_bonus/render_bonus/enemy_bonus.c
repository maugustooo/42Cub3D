...
void handle_enemy(t_game *game)
{
	...
	double transform_x = inv_det * (game->player.dir_y * (game->player.x - game->enemy.x) - game->player.dir_x * (game->player.y - game->enemy.y));
	double transform_y = inv_det * (-game->plane_y * (game->player.x - game->enemy.x) + game->plane_x * (game->player.y - game->enemy.y));
    ...
    for (int x = start_x; x < end_x; x++)
    {
		...
		int condition = transform_y > 0 && x > 0 && x < SCREEN_WIDTH && transform_y < game->z_buffer[x];
		printf("condition: %d\n", condition);
		printf("transform_y: %f\n", transform_y);
		printf("x : %d\n", x);
		printf("game->z_buffer[x]: %f\n", game->z_buffer[x]);	
